...
# Date		: 25/03/2025
#----------------------------------------------------------------------------
# CreatedBy	: ADVAN7 Offensive Security
#----------------------------------------------------------------------------
...
install_dependencies() {
    local dependencies=("anonsurf" "tor" "macchanger" "firefox-esr")
    local missing=()
    ...
    if [[ ${#missing[@]} -gt 0 ]]; then
        echo "As seguintes dependências estão ausentes: ${missing[*]}"
...
# Chama a função para verificar e instalar dependências
install_dependencies
...
while getopts "rs:ph" opt; do
    case $opt in
        ...
        s) 
            if [[ ! "$OPTARG" =~ ^([0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2}$ ]]; then
                echo "Erro: Endereço MAC inválido."
                exit 1
            fi
            COMMAND="macchanger -m $OPTARG";;
...
echo -e "\nAlterando MAC..."
{
    ...
    echo "Erro ao alterar o MAC. Verifique a interface ou as permissões."
    exit 1
}
...
# Função para verificar se o usuário está seguro
is_user_secure() {
    if anonsurf status | grep -q "enabled"; then
        return 0  # Seguro
    else
        return 1  # Não seguro
    fi
...
# Função para abrir o Firefox em modo privado
open_private_firefox() {
    local url="https://check.torproject.org"
    echo "$(date '+%Y-%m-%d %H:%M:%S') - Abrindo Firefox em modo privado com URL: $url" | tee -a "$LOG_FILE"
...
while true; do
    ...
    # Verifica se o usuário está seguro e abre o Firefox
    if is_user_secure; then
        echo "$(date '+%Y-%m-%d %H:%M:%S') - Usuário está seguro. Abrindo Firefox em modo privado." | tee -a "$LOG_FILE"
        open_private_firefox
    else
        echo "$(date '+%Y-%m-%d %H:%M:%S') - Usuário não está seguro. Firefox não será aberto." | tee -a "$LOG_FILE"
    fi
